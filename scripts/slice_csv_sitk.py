...
chemin_output = os.path.join(os.path.sep.join(inputFolder.split(os.path.sep)[:-1]),"Slice_csv_sitk")
...
def getSlicesSeg (inputImage):
    ...
    sagittal_len = inputImage.GetSize()[0]
    coronal_len = inputImage.GetSize()[1]
    axial_len = inputImage.GetSize()[2]

    sliceArray = []
    
    for i in range(sagittal_len):
        image_slice = inputImage[i, :, :]
        unique_values, counts = np.unique(sitk.GetArrayFromImage(image_slice), return_counts=True)
        resultats_comptage = list(zip(unique_values.astype(int), counts))
        sliceArray.append(["sagittal", i, image_slice.GetSize(), inputImage.GetSpacing(), resultats_comptage])
    ...
    for i in range(coronal_len):
        ...
        unique_values, counts = np.unique(sitk.GetArrayFromImage(image_slice), return_counts=True)
        resultats_comptage = list(zip(unique_values.astype(int), counts))
        sliceArray.append(["coronal",i,image_slice.GetSize(), inputImage.GetSpacing(), resultats_comptage])

    for i in range(axial_len):
        image_slice = inputImage[:, :, i]
        unique_values, counts = np.unique(sitk.GetArrayFromImage(image_slice), return_counts=True)
        resultats_comptage = list(zip(unique_values.astype(int), counts))
        sliceArray.append(["axial", i, image_slice.GetSize(), inputImage.GetSpacing(), resultats_comptage])
